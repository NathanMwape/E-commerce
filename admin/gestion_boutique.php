...
if(isset($_GET['action']) && $_GET['action'] == "affichage")
{
	...
	$contenu .= '<table border="1" cellpadding="5" class="table table-bordered table-striped"><tr>';
	...
	while ($ligne = $resultat->fetch_assoc())
	{
		...
		$contenu .= '<td><a href="?action=modification&id_produit=' . $ligne['id_produit'] .'"><i class="fa fa-pencil-square-o fa-3x"></i></a></td>';
		$contenu .= '<td><a href="?action=suppression&id_produit=' . $ligne['id_produit'] .'" OnClick="return(confirm(\'En étes vous certain ?\'));"><i class="fa fa-trash-o fa-3x"><i></a></td>';
		$contenu .= '</tr>';
	}
	$contenu .= '</table><br /><hr />	';
}
...
if(isset($_GET['action']) && ($_GET['action'] == 'ajout' || $_GET['action'] == 'modification'))
{
	...
	<form method="post" enctype="multipart/form-data" action="" class="form-group">
	
		<input type="hidden" id="id_produit" name="id_produit" class="form-control" value="';
		 if(isset($produit_actuel['id_produit'])) echo $produit_actuel['id_produit']; echo '" />
		<div class="col-4">
			<label for="reference">Reference</label><br />
			<input type="text" id="reference" name="reference" class="form-control" placeholder="la référence de produit" value="'; 
			if(isset($produit_actuel['reference'])) echo $produit_actuel['reference']; echo '" />
			<br />
		</div>
		<div class="col-4">
			<label for="categorie">Categorie</label><br />
			<input type="text" id="categorie" name="categorie" class="form-control" placeholder="la categorie de produit" value="'; if(isset($produit_actuel['categorie'])) echo $produit_actuel['categorie']; echo '"  /><br />
		</div>
		<div class="col-4">
			<label for="titre">Titre</label><br />
			<input type="text" id="titre" name="titre" class="form-control" placeholder="le titre du produit" value="'; if(isset($produit_actuel['titre'])) echo $produit_actuel['titre']; echo '"  /> <br />
		</div>

		<div class="col-5">
			<label for="description">Description</label><br />
			<textarea name="description" id="description" class="form-control" placeholder="la description du produit">'; if(isset($produit_actuel['description'])) echo $produit_actuel['description']; echo '</textarea><br />
		</div>
		
		<div class="col-4">
			<label >couleur</label>
			<input type="text" id="couleur" name="couleur" class="form-control" placeholder="la couleur du produit"  value="'; if(isset($produit_actuel['couleur'])) echo $produit_actuel['couleur']; echo '"/>
		</div>
		<div class="col-4"></div>
		...
		if(isset($produit_actuel))
		{
			...
			echo '<input type="hidden" class="form-control" name="photo_actuelle" value="' . $produit_actuel['photo'] . '" /><br />';
		}
		
		echo '
		<div class="col-4">
			<label for="prix">Prix</label><br />
			<input type="text" class="form-control" id="prix" name="prix" placeholder="le prix du produit"  value="'; if(isset($produit_actuel['prix'])) echo $produit_actuel['prix']; echo '" />
		</div>
		<div class="col-4">
			<label for="stock">Stock</label><br />
			<input type="text" class="form-control" id="stock" name="stock" placeholder="le stock du produit"  value="'; if(isset($produit_actuel['stock'])) echo $produit_actuel['stock']; echo '" />
		</div><br />
		
		<input type="submit" class="btn btn-primary" value="'; echo ucfirst($_GET['action']) . ' du produit"/>
	</form>';
}
